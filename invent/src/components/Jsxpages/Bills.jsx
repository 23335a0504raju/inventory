...
const InvoiceList = () => {
  ...
  const [billstotal, setBillstotal] = useState(0);  // Use state properly

  useEffect(() => {
    const totalBills = Math.max(0, data.length); // Ensure non-negative value
    setBillstotal(totalBills);
    localStorage.setItem("billstotal", totalBills);
  }, [data]);
  
  console.log("Hello:", billstotal);  // Now this will print the actual count
    ...
    <div className="container-fluid mt-4 px-lg-5">
      ...
      <div className="card shadow-lg rounded p-3">
        <div className="card-header bg-primary text-white text-center">
          <h4 className="mb-0">Manage Bills</h4>
        </div>
